...
import { AppBar, Toolbar, Button, IconButton, Box, Drawer, List, Divider, Menu, MenuItem, Badge } from '@mui/material';
...
export default function Header() {
  ...
  const { fosteredPets } = useFavoritePetsContext();
  const [canAdoptFosteredPet, setCanAdoptFosteredPet] = useState(false);

  useEffect(() => {
    const checkIfCanAdoptFosteredPets = () => {
      const adoptableFosteredPets = fosteredPets.filter(pet => pet.isAdoptable);
      setCanAdoptFosteredPet(adoptableFosteredPets.length > 0);
    };

    checkIfCanAdoptFosteredPets();
  }, [fosteredPets]);
            ...
            {user && !loading && (
              <>
                ...
                <HeaderNavButton to="/pets/search" icon={<PageViewIcon />} label="Search" activePath={location.pathname} />
                     {/* Favorite Pets link with conditional marker */}
                     <HeaderNavButton to="/users/favoritepets" icon={<FavoriteIcon />} label="Favorite Pets" activePath={location.pathname}>
                  {canAdoptFosteredPet && (
                    <Badge color="secondary" variant="dot" sx={{ position: 'absolute', top: -6, right: -6 }} />
                  )}
                </HeaderNavButton>
                {isAdmin && (
                  <>
        ...
        <List sx={{ backgroundColor: 'var(--secondary)', height: '100vh' }}>
          {user && !loading && (
            <>
              ...
              <HeaderDrawerListItem to="/users/favoritepets" icon={<FavoriteIcon />} label="Favorite Pets" onClick={() => setDrawerOpen(false)}>
                {canAdoptFosteredPet && (
                  <Badge color="secondary" variant="dot" sx={{ position: 'absolute', top: -6, right: -6 }} />
                )}
